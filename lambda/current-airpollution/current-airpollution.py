...
def lambda_handler(event, context):
    ...
    for city in cities:
        ...
        key = f"airpollution/current/{city}-{datetime.utcnow().isoformat()}.json"
        s3.put_object(Bucket=os.environ["S3OWBucket"], Key=key, Body=json.dumps(data), ContentType='application/json')
    return {'statusCode': 200, 'body': json.dumps({"message": "Air pollution (current) stored", "cities": cities})}
