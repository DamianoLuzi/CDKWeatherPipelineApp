...
def lambda_handler(event, context):
    ...
    api_key = get_api_key()
    for city in cities:
        url = f"http://api.openweathermap.org/data/2.5/forecast?q={urllib.parse.quote(city)}&appid={api_key}&units=metric"
        with urllib.request.urlopen(url) as response:
            data = json.loads(response.read().decode())
        key = f"weather/forecast/{city}-{datetime.utcnow().isoformat()}.json"
        s3.put_object(Bucket=os.environ["S3OWBucket"], Key=key, Body=json.dumps(data), ContentType='application/json')
    return {'statusCode': 200, 'body': json.dumps({"message": "Forecast stored", "cities": cities})}
